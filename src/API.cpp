SmallParsimony(shared_ptr<int> idx_arr, shared_ptr<int> children_arr,
   ...
   * 		(all leaf id is at the very begining; total length is (n+1 (i.e. 1 is
   * root)), with last one is root's index; leaves indices is -1) children_arr:
   * store children node id. (length are always multiple of 2) e.g. idx_arr =
   * [-1, -1, -1,-1, 0, 2, 4]; children_arr=[0,1, 2,3, 4,5]
   *
