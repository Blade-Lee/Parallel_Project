...
public:
    ...
    shared_ptr<string> string_list; // N, assign each of node a string finally (problem of cirle reference???)
    
    SmallParsimony(shared_ptr<int> idx_arr, shared_ptr<int> children_arr, shared_ptr<char> char_list, int num_char_trees, int num_nodes){
    ...
    int run_small_parsimony_char(char* char_list){ // use current char list and global tree structure to calculate
        ...
        // return: the small parsimony score of the char tree and also write the assigned chars to the global char_list
        
        // local allocation
        // using unique_ptr inside here, cause unique_ptr has partial definition for unique_ptr
        unique_ptr<int[]> s_v_k(new int[this->num_nodes * 4]); // indicate the score of node v choosing k char
        unique_ptr<unsigned char[]> tag (new unsigned char[this->num_nodes]); // indicate if the noed i is ripe
        unique_ptr<unsigned char[]> back_track_arr (new unsigned char[this->num_nodes * 8]); // indicate for each node, chosen a char of 4, what is the best char for its left & right children.
        ...
        for(int i = 0; i < this->num_nodes; i++){
            ...
            for(int j = 0; j < 4; j++){
                s_v_k.get()[bias + j] = infinity * int(node_idx == -1 && j != leaf_char); // if it is a leaves && it is not the leave char, assign it to infinity
            }
