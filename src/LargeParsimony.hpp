...
public:
    ...
    // for internal use
    shared_ptr<int> cur_unrooted_undirectional_tree;// must have a copy of unrooted_undirectional_tree for internal exchange use
    ...
    shared_ptr<int> edges; // for get_edges_from_unrooted_undirectional_tree() use
    shared_ptr<bool> visited; // for get_edges_from_unrooted_undirectional_tree use
    ...
    LargeParsimony(shared_ptr<int> unrooted_undirectional_tree, shared_ptr<int>unrooted_undirectional_idx_arr, shared_ptr<char> rooted_char_list, int num_nodes, int num_leaves, int num_char_trees){
        ...
        this->cur_unrooted_undirectional_tree = shared_ptr<int>(new int[unrooted_undirectional_tree_len],
                                      [](int* p) { delete[] p; });
        for(int i = 0; i < unrooted_undirectional_tree_len; i++){
            this->cur_unrooted_undirectional_tree.get()[i] = this->unrooted_undirectional_tree.get()[i];
        }
        ...
        this->rooted_directional_tree = shared_ptr<int>(new int[(num_nodes + 1 - num_leaves) * 2],
                                      [](int* p) { delete[] p; });
        this->rooted_directional_idx_arr = shared_ptr<int>(new int[num_nodes + 1],
                                      [](int* p) { delete[] p; });
        int rooted_char_list_len = (num_nodes + 1) * num_char_trees;
        // need to get the char list copy below
        this->cur_rooted_char_list = shared_ptr<char>(new char[rooted_char_list_len],
                                      [](char* p) { delete[] p; });
        ...
        this->visited = shared_ptr<bool>(new bool[this->num_nodes], [](bool* p){delete[] p;});
    }
    ~LargeParsimony(){
        cout<<"Large Parsimony deconstructed."<<endl;
    }
    
    /* 
        get a new (unrooted_undirectional_tree,) from old (unrootedundirectional_tree) by interchange of an given edge
    ...
    shared_ptr<int> get_edges_from_unrooted_undirectional_tree(){
        ...
        for(int i = this->num_leaves; i < this->num_nodes; i++){
            ...
            for(int j = b_idx; j < b_idx + 3; j++){
                ...
                if(b >= this->num_leaves && !this->visited.get()[b]){ // edge(a-b) is an internal edge
    ...
    void run_large_parsimony(){
        ...
        shared_ptr<int> new_unrooted_directional_tree = shared_ptr<int>(new int[unrooted_undirectional_tree_len],
                                      [](int* p) { delete[] p; });
        for(int i = 0; i < unrooted_undirectional_tree_len; i++){
            new_unrooted_directional_tree.get()[i] = this->unrooted_undirectional_tree.get()[i];
        }
        while(new_score < this->min_large_parsimony_score){
            ...
            shared_ptr<int> tmp_unrooted_undirectional_tree = this->unrooted_undirectional_tree;
            ...
            shared_ptr<int> edges = get_edges_from_unrooted_undirectional_tree(); // write to this->edges; this->visited
            ...
            for(int i = 0; i < this->num_edges; i+=2){
                ...
                a_child = a_child==b? this->unrooted_undirectional_tree.get()[a_child_idx + 1] : a_child;
                ...
                for(int j = 0; j < 2; j++){ // exchange b's j_th child in unrooted & undirectional tree
                    if(j){
                        for(int k = 2; k >= 0; k--){
                            b_child = this->unrooted_undirectional_tree.get()[b_child_idx + k];
                            if(b_child != a)
                                break;
                        }
                    }else{
                    ...
                    // begin interchange
                    nearest_neighbor_interchage(a, b, a_child, b_child); // writed to this->cur_unrooted_undirectional_tree
                    ...
                    make_tree_rooted_directional();// write to this->cur_unrooted_undirectional_tree; this->rooted_directional_idx_arr; this->rooted_directional_tree;
                    
                    // cout<<"reach here after"<<endl;
                    
                    // need to get the char list copy below
                    int rooted_char_list_len = (this->num_nodes+1) * this->num_char_trees;
                    for(int i = 0; i < rooted_char_list_len; i++){
                        this->cur_rooted_char_list.get()[i] = this->rooted_char_list.get()[i];
                    }
                    // run small parsimony
                    small_parsimony = make_shared<SmallParsimony>(this->rooted_directional_idx_arr, this->rooted_directional_tree, this->cur_rooted_char_list, this->num_char_trees, this->num_nodes + 1);
                    small_parsimony.get()->run_small_parsimony_string();
                    // cout<<"new_score is: "<<new_score<<" small_parsimony->total_score is: "<<small_parsimony.get()->total_score<<endl;
                    if(small_parsimony.get()->total_score < new_score){// compare
                        ...
                        new_unrooted_directional_tree = this->cur_unrooted_undirectional_tree;
                        this->cur_unrooted_undirectional_tree = tmp_unrooted_undirectional_tree;
